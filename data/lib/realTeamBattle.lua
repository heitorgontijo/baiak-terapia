...
realTeamBattle = {
	...
	teleportPosition = {x = 167, y = 48, z = 7},
	exit = {x = 155, y = 54, z = 7},
	ips = {}
...
function realTeamBattle.winEvent(team,ammount)
	...
	for _, pid in pairs(realTeamBattle.getPlayers()) do
		if getPlayerStorageValue(pid,realTeamBattle.teamStorage) == team then
			doPlayerAddItem(pid, 6527, 700)
			doPlayerSendTextMessage(pid,25,"[Team Battle] Parabéns por vencer o evento, você ganhou 700 Event Coins!")
		end
		realTeamBattle.cleanPlayer(pid)
		doTeleportThing(pid,realTeamBattle.exit)
	end
	...
	doBroadcastMessage("[Team Battle] "..ammount.." jogadores do time "..teams[team].." venceram o evento e ganharam 120 Event Coins e 200kk!")
end
...
function realTeamBattle.empate(team1,team2)
	...
	for _, pid in pairs(realTeamBattle.getPlayers()) do
		...
		if sto == team1 or sto == team2 then
			doPlayerAddItem(pid, 6527, 700)
			doPlayerSendTextMessage(pid,25,"[Team Battle] Parabéns por vencer o evento, você ganhou 700 Event Coins e 200kk!")
		end
		realTeamBattle.cleanPlayer(pid)
		doTeleportThing(pid,realTeamBattle.exit)
	end
